"""
Валидация входных данных для модели kling/v2-1-pro
"""

def validate_kling_v2_1_pro_input(data: dict) -> tuple[bool, list[str]]:
    """
    Валидирует входные данные для модели kling/v2-1-pro
    ...
    # 6. Проверка tail_image_url (опциональный)
    if 'tail_image_url' in data and data['tail_image_url']:
        tail_image_url = str(data['tail_image_url'])
        if len(tail_image_url) == 0:
            errors.append("[ERROR] Параметр 'tail_image_url' не может быть пустой строкой, если указан")
        # Проверяем, что это похоже на URL (базовая проверка)
        if tail_image_url and not (tail_image_url.startswith('http://') or tail_image_url.startswith('https://')):
...
test_data = {
    ...
    "tail_image_url": "Click to upload or drag and drop"  # Опциональный, может быть placeholder
}
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Валидация входных данных для kling/v2-1-pro")
    print("=" * 60)
    print()
    ...
    print(f"  tail_image_url: '{test_data['tail_image_url']}' (опциональный)")
    print()
    
    is_valid, errors = validate_kling_v2_1_pro_input(test_data)
    ...
    if is_valid:
        ...
        # tail_image_url
        if test_data.get('tail_image_url') and test_data['tail_image_url'] not in ["Click to upload or drag and drop", ""]:
            print(f"  - tail_image_url: указан [OK]")
        else:
        ...
        print("  - duration влияет на цену: 5 секунд (50 кредитов), 10 секунд (100 кредитов)")
        print("  - cfg_scale определяет, насколько близко модель следует промпту (0-1, шаг 0.1)")
        print("  - tail_image_url - опциональное изображение для конца видео")
    ...
    # tail_image_url - только если указан и это реальный URL (не placeholder)
    if test_data.get('tail_image_url'):
        tail_url = test_data['tail_image_url']
        # Проверяем, что это не placeholder
        if tail_url and tail_url not in ["Click to upload or drag and drop", ""] and (tail_url.startswith('http://') or tail_url.startswith('https://')):
            api_data['tail_image_url'] = tail_url
