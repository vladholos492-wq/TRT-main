"""
Валидация входных данных для модели bytedance/v1-lite-image-to-video
"""

def validate_bytedance_v1_lite_image_to_video_input(data: dict) -> tuple[bool, list[str]]:
    """
    Валидирует входные данные для модели bytedance/v1-lite-image-to-video
    ...
    # 8. Проверка end_image_url (опциональный)
    if 'end_image_url' in data and data['end_image_url']:
        end_image_url = str(data['end_image_url'])
        if len(end_image_url) == 0:
            errors.append("[ERROR] Параметр 'end_image_url' не может быть пустой строкой, если указан")
        # Проверяем, что это похоже на URL (базовая проверка)
        if end_image_url and not (end_image_url.startswith('http://') or end_image_url.startswith('https://')):
            # Это не ошибка, так как может быть placeholder типа "Click to upload"
            pass
...
test_data = {
    ...
    "seed": "-1",  # Указан -1 для случайного
    "enable_safety_checker": None,  # Не указан - опциональный
    "end_image_url": "Click to upload or drag and drop"  # Опциональный, может быть placeholder
}
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Валидация входных данных для bytedance/v1-lite-image-to-video")
    print("=" * 60)
    print()
    ...
    print(f"  end_image_url: '{test_data['end_image_url']}' (опциональный)")
    print()
    
    is_valid, errors = validate_bytedance_v1_lite_image_to_video_input(test_data)
    ...
    if is_valid:
        ...
        if test_data.get('seed'):
            try:
                ...
                if seed_val == -1:
                    print(f"  - seed: {seed_val} - случайный seed [OK]")
                else:
        ...
        # end_image_url
        if test_data.get('end_image_url') and test_data['end_image_url'] not in ["Click to upload or drag and drop", ""]:
            print(f"  - end_image_url: указан [OK]")
        else:
        ...
        print("  - end_image_url - опциональное изображение для конца видео (defaults to None)")
    else:
    ...
    # end_image_url - только если указан и это реальный URL (не placeholder)
    if test_data.get('end_image_url'):
        end_url = test_data['end_image_url']
        # Проверяем, что это не placeholder
        if end_url and end_url not in ["Click to upload or drag and drop", ""] and (end_url.startswith('http://') or end_url.startswith('https://')):
            api_data['end_image_url'] = end_url
